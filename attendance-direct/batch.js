...
export async function runTomorrowBatch({pool=getPool(),requestFn=request,now=new Date(),commit=false,maxSnapshotAgeMs=30*60*1000}={}){
  const date=chileDate(now,1);
  ...
  if(!rows.length)throw Error('tomorrow_snapshot_missing');
  const syncedAt=new Date(rows[0].synced_at);
  if(!Number.isFinite(syncedAt.valueOf()) || now.valueOf()-syncedAt.valueOf()>maxSnapshotAgeMs)throw Error('tomorrow_snapshot_stale');
  const raw=rows[0].appointments;
  if(!Array.isArray(raw))throw Error('tomorrow_snapshot_invalid');
  ...
  for(const a of items){
    try{
      const key=`daily20-${date.replaceAll('-','')}-${a.id}-${a.fingerprint.slice(0,12)}`;
      const result=await requestFn(a,{trial:false,key,actor:'daily-20h',now});
